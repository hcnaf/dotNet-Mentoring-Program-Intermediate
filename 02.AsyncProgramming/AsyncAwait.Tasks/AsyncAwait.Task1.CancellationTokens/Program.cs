...
{
    private static Tuple<Task, CancellationTokenSource, int> currentOperation;
    ...
    private static void CalculateSum(int n)
    {
        ...
        {
            var taskToCancel = currentOperation.Item1;

            if (taskToCancel.Status != TaskStatus.RanToCompletion)
            {
                currentOperation.Item2.Cancel();
                Console.WriteLine($"Sum for {currentOperation.Item3} cancelled...");
            }
        }
        ...
        var printSumTask = sumTask.ContinueWith(t =>
        {
            Console.WriteLine($"Sum for {n} = {t.Result}.");
            Console.WriteLine("Enter N: ");
        }, cts.Token);

        currentOperation = new (printSumTask, cts, n);
